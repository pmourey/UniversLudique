...
class HoldemRoom implements GameRoom
{
    ...
    {
        // Placeholder
        $this->broadcast([ 'type' => 'notice', 'payload' => [ 'message' => "Texas Hold'em: non implémenté pour le moment." ] ]);
    }

    public function handleAction(ConnectionInterface $from, $action, array $params = array())
    {
        $this->broadcast([ 'type' => 'notice', 'payload' => [ 'message' => "Texas Hold'em: action non implémentée." ] ]);
    }
